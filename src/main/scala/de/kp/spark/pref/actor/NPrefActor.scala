...
class NPrefActor(@transient val sc:SparkContext) extends BaseActor {
    ...
    case req:ServiceRequest => {
      ...
      if (missing == false) {
        ...
        try {
          ...
          val nprefs = NPrefBuilder.build(dataset)
          savePreferences(nprefs)
  ...
  /**
   * This method is responsible for persisting user item
   * preferences
   */
  private def savePreferences(nprefs:RDD[NPref]) {
    // TODO
  }
}